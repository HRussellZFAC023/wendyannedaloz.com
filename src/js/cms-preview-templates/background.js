...
export default class BackgroundPreview extends React.Component {
  render() {
    ...
    const entryBackground = entry.getIn(["data", "background"]);
    const background = entryBackground ? entryBackground.toJS() : [];
    
    return <div>
      <Jumbotron image={image} title={entry.getIn(["data", "title"])} />
      <div className="bg-off-white pv4">
        <div className="mw7 center ph3 pt4">
          {background.map(({text, heading, imageUrl}, i) =>
